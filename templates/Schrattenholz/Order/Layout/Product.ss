			...
			<div class="offset-md-1 col-md-7 col-sm-6 pt-3">
				...
				<form id="product-config">
					...
					<div class="col-md-12" id="vac_container" style="<% if $Vacuum %>display:block;<% else %>display:none;<% end_if %>">
						...
						<input type="checkbox" class="styledcheckbox" id="vac" data-price="$OrderConfig.VacuumPrice" <% if $Top.loadSelectedParameters.Vac=="1" %>checked="checked"<% end_if %> onchange="refreshSelectedProduct('attributeChange')" /><label for="vac">Die einzelnen Stücke vakuumieren (Aufpreis pro Stück {$OrderConfig.VacuumPrice}€*)<label>
					</div>
					<div class="col-12">
							<h3 id="price"></h3>
						</div>
...
<script>
    function getRenderedBasket(){
        jQuery.ajax({
            url: "{$Link}/getRenderedBasket",
